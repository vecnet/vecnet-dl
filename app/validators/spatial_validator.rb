class SpatialValidator < ActiveModel::Validator
  ...
  def validate(record)
    ...
    if nothing_present?(record.latitude,record.longitude)
      ...
      record.errors[:latitude] << "Invalid Spatial Data. latitude is blank" if record.latitude.blank?
      record.errors[:longitude] << "Invalid Spatial Data. longitude is blank" if record.longitude.blank?
    end
  end
