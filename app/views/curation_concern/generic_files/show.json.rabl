...
node(:metadata) do
  ...
  [:description, :depositor, :related_url, :based_near, :part_of, :creator,
   ...
   :resource_type, :identifier, :language, :spatials, :temporals, :species, :conforms_to].each do |term|
...
node(:charactization) do
  ...
  [:mime_type, :format_label,  :file_size,  :last_modified,  :filename,
          ...
          :markup_language,  :duration,  :bit_depth,  :sample_rate, :channels,  :data_format,  :offset].each do |term|
    value = curation_concern.send(term)
    result[term] = value if value.present?
  end
  result
