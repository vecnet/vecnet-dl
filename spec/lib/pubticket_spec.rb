...
-----END PUBLIC KEY-----
  ...
  describe ".initialize" do
    ...
    it 'should parse the signature, if present' do
      ...
      pt.ticket.should == ticket
      pt.text.should == fields
      pt.signature.should == sig
      pt.signature_valid?(dsa_public_key).should be_false
    end
    it 'validates a good signature' do
      pt = PubTicket.new(ticket_dsa)
      pt.signature_valid?(dsa_public_key).should be_true
    end
    it 'parses the fields correctly' do
      pt = PubTicket.new(ticket)
      pt.uid.should == "xxx"
      pt.valid_until.should == Time.at(123456789)
      pt.tokens.should == ""
      pt.user_data.should == ""
    end
  end
